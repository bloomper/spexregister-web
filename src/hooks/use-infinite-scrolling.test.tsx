...
describe('useInfiniteCursor', () => {
    ...
    it('re-seeds when the caller supplies genuinely new initial data', async () => {
        ...
        const {result, rerender} = renderHook(() =>
            useInfiniteCursor<Item>({
                ...
                initialPageInfo: {hasNextPage: false, hasPreviousPage: false, startCursor: null, endCursor: seed.endCursor},
            }),
        );
    ...
    it('seeds from initial items and page info without fetching', async () => {
        ...
        const {result} = renderCursor({fetchPageAction, pageSize: 5, getKeyAction: getKey, initialItems, initialPageInfo});
