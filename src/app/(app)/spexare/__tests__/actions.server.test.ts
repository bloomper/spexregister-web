...
vi.mock('@/lib/spexare/address', () => ({create: vi.fn(), update: vi.fn(), del: vi.fn(), addressFormSchema: {parse: (d: unknown) => d}}));
vi.mock('@/lib/spexare/consent', () => ({create: vi.fn(), update: vi.fn(), del: vi.fn(), consentFormSchema: {parse: (d: unknown) => d}}));
vi.mock('@/lib/spexare/membership', () => ({create: vi.fn(), update: vi.fn(), del: vi.fn(), membershipFormSchema: {parse: (d: unknown) => d}}));
vi.mock('@/lib/spexare/tagging', () => ({create: vi.fn(), del: vi.fn()}));
vi.mock('@/lib/spexare/toggle', () => ({create: vi.fn(), update: vi.fn(), del: vi.fn(), toggleFormSchema: {parse: (d: unknown) => d}}));
...
vi.mock('@/lib/spexare/activity/task-activity/actor', () => ({create: vi.fn(), update: vi.fn(), del: vi.fn(), actorFormSchema: {parse: (d: unknown) => d}}));

import * as actions from '@/app/(app)/spexare/actions.server';
import {create, update, del, getPaged} from '@/lib/spexare';
...
describe('spexare sub-resource create strips the form typeId in favour of the positional arg', () => {
    it.each([
        ...
        ['membership', () => actions.createMembershipAction('s1', 't1', {typeId: 'FORM', year: 2000}), createMembership],
