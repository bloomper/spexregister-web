...
vi.mock('@/components/spexare/spexare-view.client', () => ({
    SpexareView: ({spexare}: {spexare: {firstName: string}}) => <div data-testid="spexare-view">{spexare.firstName}</div>,
}));
...
describe('SpexareCard', () => {
    it('renders the name and nickname', () => {
        render(<SpexareCard spexare={spexare} index={0} isMe={false} canEdit={false} onSelect={vi.fn()} onEdit={vi.fn()}/>);
    ...
    it('fires onSelect when the card body is clicked', async () => {
        ...
        render(<SpexareCard spexare={spexare} index={0} isMe={false} canEdit={false} onSelect={onSelect} onEdit={vi.fn()}/>);
...
describe('SpexareViewDialog', () => {
    it('renders the full view and closes via the footer button', async () => {
        const onClose = vi.fn();
        render(<SpexareViewDialog open onClose={onClose} full={spexare} isLoading={false} countries={[]} isMe={false}/>);
    ...
    it('renders nothing when closed', () => {
        render(<SpexareViewDialog open={false} onClose={vi.fn()} full={spexare} isLoading={false} countries={[]} isMe={false}/>);
        expect(screen.queryByTestId('spexare-view')).not.toBeInTheDocument();
    });
});
