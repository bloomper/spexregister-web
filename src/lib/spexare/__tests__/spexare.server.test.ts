...
import {
    ...
    search,
    uploadImage
} from "@/lib/spexare/spexare.server";
import {ImpexType, ReportType, SortDirection} from "@/gql/schema";
...
vi.mock("@/lib/axios.server", () => ({
    default: {
        put: (...a: unknown[]) => axiosPut(...a),
        delete: (...a: unknown[]) => axiosDelete(...a)
    }
}));
...
describe("spexare get/export", () => {
    ...
    it("exp forwards ids/filter/type/reportType and returns the job", async () => {
        ...
        expect(query.mock.calls[0][1]).toEqual({
            ...
            filter: "f",
            type: ImpexType.Pdf,
            reportType: ReportType.PdfPlatoonList
        });
    });
});
