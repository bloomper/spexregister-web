...
import {getPaged} from '@/lib/tag/tag.server';
...
describe('tag.server (factory config binding)', () => {
    it('applies tag-specific defaults and maps the connection', async () => {
        toPromise.mockResolvedValue({data: {tagPaged: connection([{id: '1'}], false, null)}});
        const page = await getPaged({});
        expect(page.items).toEqual([{id: '1'}]);
        const vars = query.mock.calls[0][1] as Record<string, unknown>;
        expect(vars).toMatchObject({sort: ['name'], direction: SortDirection.Desc, filter: ''});
    });
